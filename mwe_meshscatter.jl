...
# ╔═╡ 39fc0b36-3c86-11ec-2ab1-91f1bb642854
begin
	using WGLMakie, JSServe
	Page()
end

# ╔═╡ 17cfc32c-d205-41be-b7bb-3d6b72efa8b7
using StaticArrays, PlutoUI, Rotations

# ╔═╡ ec59b31f-24e1-4960-9635-22fa6422204a
md"#### Set up nodes for meshscatter parameters"

# ╔═╡ c8e72e4d-13c3-40e5-aeb6-3d00081e698f
positions = Node([Vec3f0(0,0,0)])

# ╔═╡ 3f4e6214-7673-41a9-84fc-4dfb4e2124b8
markersizes = Node([Vec3f0(1,1,1)])

# ╔═╡ 8b64baa4-6624-49ff-8f51-d46384a9a3db
function yaw_to_quat(yaw::Real)
	r = RotZ(yaw)
	q = UnitQuaternion(r)
	[q.x, q.y, q.z, q.w]
end

# ╔═╡ bbf0d092-b50b-4626-a9f3-12f588f99396
#yaw(y) = Vec3f([0, 0, y])
yaw(θ) = Makie.qrotation(SVector(0, 0, 1), θ)

# ╔═╡ e0dd07d5-033c-4dde-ab1e-c8e10bf6cdb0
rotations = Node([yaw(0)])
#rotations = Node([Makie.qrotation(SVector(0, 0, 0), 1)])
#rotations = Node([Vec3f0(0,0,0)])

# ╔═╡ 30b0553f-5db4-4f71-8bd0-8a0f60c29c15
framerate = 20.0

# ╔═╡ 25d23e42-2acb-422b-b58a-ac476d75215d
@bind clock PlutoUI.Clock(;interval=1/framerate)

# ╔═╡ ae3de12c-e32b-4564-9406-50db220a560c
clock

# ╔═╡ 13a8f1d5-aa17-4fba-becd-bf3f820357dc
t = Node{Real}(0.0)

# ╔═╡ db8f2928-6b21-427b-854a-6112bb97b18a
t[] = clock/framerate

# ╔═╡ 4797949b-28ea-4506-bf6c-b63d26d64265
t

# ╔═╡ e1efc3be-8380-4995-8da5-f61682472259
scene = Scene(resolution=(1000, 1000), transparency = true)

# ╔═╡ b13be021-07eb-42dd-a0c4-3015fce19c4b
plot!(scene, Vec3f(1,1,1))

# ╔═╡ 2060716c-3b11-4472-8238-ca2718b790cd
meshscatter!(scene, positions, marker=FRect3D(Vec3f(0), Vec3f(1)), markersize=markersizes, rotations = rotations, color=:Red, camera=cam3d!)

# ╔═╡ b154bc00-dd6a-4684-888b-dce182b5c230
on(t) do t
		r = 2
		positions.val = [r .* Vec3f0(cos(t), sin(t), 0)]   # delay update...
        markersizes.val = [Vec3f0(1, 1, 1+0.4*cos(t))]  # delay update...
        rotations[] = [yaw(t)]        # ...and NOW trigger all changes...?        
end
